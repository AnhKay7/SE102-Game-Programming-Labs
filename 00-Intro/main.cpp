...
#define WINDOW_ICON_PATH L"ball.webp" 
...
#define TEXTURE_PATH_BRICK L"ball.webp"
#define BRICK_START_X WINDOW_WIDTH / 2
#define BRICK_START_Y WINDOW_HEIGHT /2

#define BRICK_START_VX 0.3f
#define BRICK_START_VY 0.3f
...
#define TEXTURE_PATH_PADDLE L"whiteTexture.webp"
...
D3DX10_SPRITE spritePaddle;

Paddle leftPaddle = { 0, 0, 16.0f, 80.0f, 0.3f };
Paddle rightPaddle = { 0, 0, 16.0f, 80.0f, 0.3f };
...
void InitDirectX(HWND hWnd)
{
	...
	*/
	...
	leftPaddle.x = 0 + leftPaddle.width / 2;
	rightPaddle.x = BackBufferWidth - rightPaddle.width / 2;
	leftPaddle.y = BackBufferHeight / 2.0f;
	rightPaddle.y = BackBufferHeight / 2.0f;
...
void LoadResources()
{
	...
	HRESULT paddleResult = D3DX10CreateShaderResourceViewFromFile(
		pD3DDevice,
		TEXTURE_PATH_PADDLE,
		NULL,
		NULL,
		&spritePaddle.pTexture,
		NULL
	);

	if (FAILED(paddleResult))
	{
		DebugOut(L"[ERROR] Cannot load \n", TEXTURE_PATH_PADDLE);
		return;
	}
	...
	spritePaddle.TexCoord.x = 0;
	spritePaddle.TexCoord.y = 0;
	spritePaddle.TexSize.x = 1.0f;
	spritePaddle.TexSize.y = 1.0f;
	spritePaddle.ColorModulate = D3DXCOLOR(1.0f, 0.0f, 0.0f, 1.0f);
...
*/
bool isBrickHitPaddle(const Paddle &paddle) {

	return 
		brick_x <= paddle.x + paddle.width / 2 
		&& brick_x >= paddle.x - paddle.width / 2
		&& brick_y <= paddle.y + paddle.height / 2
		&& brick_y >= paddle.y - paddle.height / 2;
}
float clampPaddle(const Paddle& paddle) {

	float top_edge = BackBufferHeight - paddle.height / 2;
	if (paddle.y > top_edge)
		return top_edge;

	float low_edge = paddle.height / 2;
	if (paddle.y < low_edge)
		return low_edge;
...
void Update(DWORD dt)
{
	...
	float low_edge = BackBufferHeight - BRICK_HEIGHT;

	bool wHeld = (GetAsyncKeyState('W') & 0x8000) != 0;
	bool sHeld = (GetAsyncKeyState('S') & 0x8000) != 0;
	bool upKeyHeld = (GetAsyncKeyState(VK_UP) & 0x8000) != 0;
	bool downKeyHeld = (GetAsyncKeyState(VK_DOWN) & 0x8000) != 0;

	leftPaddle.y += leftPaddle.speed * dt * (sHeld - wHeld);
	rightPaddle.y += rightPaddle.speed * dt * (downKeyHeld - upKeyHeld);

	leftPaddle.y = clampPaddle(leftPaddle);
	rightPaddle.y = clampPaddle(rightPaddle);

	if (isBrickHitPaddle(leftPaddle) || isBrickHitPaddle(rightPaddle)) {
...
void DrawPaddle(const Paddle& paddle)
{
	...
	D3DXMatrixScaling(&matScaling, paddle.width, paddle.height, 1.0f);

	D3DXMATRIX matTranslation;
	D3DXMatrixTranslation(&matTranslation, paddle.x, (BackBufferHeight - paddle.y), 0.1f);

	spritePaddle.matWorld = (matScaling * matTranslation);

	spriteObject->DrawSpritesImmediate(&spritePaddle, 1, 0, 0);
}
void Render()
{
	if (pD3DDevice != NULL)
...
void Cleanup()
{
	if (spritePaddle.pTexture != NULL)
	{
		spritePaddle.pTexture->Release();
		spritePaddle.pTexture = NULL;
	}
